...
function startJudge(submitID,userID,callback) {
  loadInfo(submitID,userID,function(loadInfoErr,loadInfoResult) {
    if(loadInfoErr) {
      callback(loadInfoErr);
      return;
    }
    var problemInfo=loadInfoResult.problemInfo, submitInfo=loadInfoResult.submitInfo;
    sql.updateJudgeResult(submitID,problemInfo.problem_id,userID,1,function(updateCompileErr) {
      if(updateCompileErr) {
        callback(updateCompileErr);
        return;
      }
      envSetup(submitID,userID,submitInfo.lang);
      compile(submitID,userID,submitInfo.lang,function(compileResult) {
        if(compileResult===0) {
          ...
          winston.error('['+submitID+'] Error while starting compile');
          sql.updateCompileError(submitID,'Error while starting compile.',function(updateCompileErr,updateCompileResult) {
            if(updateCompileErr) {
              winston.info(updateCompileErr);
              callback(updateCompileErr);
              return;
            }
            sql.updateJudgeResult(submitID,problemInfo.problem_id,userID,9,function(updateJudgeErr) {
              if(updateJudgeErr) {
                winston.info(updateJudgeErr);
                callback(updateJudgeErr);
                return;
          ...
          sql.updateJudgeResult(submitID,problemInfo.problem_id,userID,2,function(updateJudgeErr) {
            ...
            run(submitInfo,userID,problemInfo,1,0,0,function(judgeErr) {
              if(judgeErr) {
...
function loadInfo(submitID,userID,callback) {
  var result={};
  sql.submitInfo(submitID,function(submitInfoErr,submitResult) {
    if(submitInfoErr) {
      callback(submitInfoErr,null);
      return;
    }
    result.submitInfo = submitResult[0];
    sql.problemInfo(result.submitInfo.problem_id,function(problemInfoErr,problemResult) {
      if(problemInfoErr) {
        callback(problemInfoErr,null);
        return;
      }
      result.problemInfo = problemResult[0];
      callback(null,result);
    });
  });
}
function run(submitInfo,userID,probInfo,caseNo,mem,time,callback) {
  if(caseNo > probInfo.case_count) {
    ...
    runEnvSetup(submitInfo.submit_id,userID,submitInfo.lang);
    cprocess.execFile(path.resolve(rootDir,'./judge_tmp/'+submitInfo.submit_id+'/run'),compile_info.getRunArgs(submitInfo.lang,submitInfo.submit_id,probInfo.memory_limit,probInfo.time_limit,path.resolve(rootDir,'./judge_tmp/'+submitInfo.submit_id)), function(error,stdout,stderr) {
      console.log(stdout);
      ...
      else run(submitInfo,userID,probInfo,caseNo+1,mem,time,callback);
    });
  }
}
function compile(submitID,userID,lang,callback) {
  compileEnvSetup(submitID,userID,lang);
  winston.log('['+submitID+'] Compiling');
  if(!compile_info.validLanguage(lang)) {
    callback(2);
    return;
  }
  else if(compile_info.needsCompile(lang)) {
    console.log(JSON.stringify(compile_info.getCompileArgs(lang,submitID,path.resolve(rootDir,'./judge_tmp/'+submitID))));
    cprocess.execFile(path.resolve(rootDir,'./judge_tmp/'+submitID+'/build'),compile_info.getCompileArgs(lang,submitID,path.resolve(rootDir,'./judge_tmp/'+submitID)), function(error,stdout,stderr) {
      console.log(stdout);
      console.error(stderr);
      if(error) {
        ...
        console.error('Error while reading compile result');
        callback(2);
        return;
      }
...
function envSetup(submitID,userID,lang) {
  fs.mkdirSync(path.resolve(rootDir,'./judge_tmp/'+submitID));
...
function compileEnvSetup(submitID,userID,lang) {
  console.log(path.resolve(rootDir,'./judge_bin/build/'+lang));
  console.log(path.resolve(rootDir,'./judge_tmp/'+submitID+'/build'));
  fs.copySync(path.resolve(rootDir,'./judge_bin/build/'+lang),path.resolve(rootDir,'./judge_tmp/'+submitID+'/build'));
}
function runEnvSetup(submitID,userID,lang) {
  ...
  fs.copySync(path.resolve(rootDir,'./judge_bin/run/'+lang),path.resolve(rootDir,'./judge_tmp/'+submitID+'/run'));
}
function envCleanUp(submitID,userID) {
